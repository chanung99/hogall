...
<script
	src="${pageContext.request.contextPath }/resources/js/lightslider.js"></script>
<link rel="stylesheet"
	href="${path}/resources/css/lightslider.css?after" />
		...
		<article id="content_article">
			<div id="conetnt_box">
							...
							<div class="clearfix">
								<ul id="image-gallery" class="gallery list-unstyled cS-hidden">
									<c:forEach items="${image }" var="i">
										<li
											data-thumb="${pageContext.request.contextPath}/resources/product/${i.reFileName}"
											style="width: 88px;"><img id="productImage"
											src="${pageContext.request.contextPath}/resources/product/${i.reFileName}" />
										</li>
									</c:forEach>
					...
					<div class="productDetail_information fl">
						<div class="information_wrap">
							...
							<table>
								...
								<tr>
									<th>즉시 구매가</th>
									<td><c:if test="${product.buyNowWhether=='Y' }">
											<p style="color: green;">${product.buyNowPrice }</p>
										</c:if> <c:if test="${product.buyNowWhether=='N' }">
								...
								<tr>
									...
									<td>${pCount }<!-- <button type="button" id="bidding" data-target="#myModalTwo" data-toggle="modal" class="">입찰자 내역</button> -->
										<a href="#ex1" class="bidBtn" rel="modal:open"><button
												id="bidding" type="button">입찰자 내역</button></a>
										...
										<div id="ex1" class="modal">
											<div>
												<table>
													...
													<c:forEach var="p" items="${ppap }" varStatus="vs">
														...
														<tr data-memberId='${p.memberId }'>
															<td class="pMemberId">${p.memberId }</td>
															<td>${p.curPrice }</td>
															<td class="enrollDate">${p.enrollDatee }</td>
															<td><button type="button" class="messageBtn">쪽지보내기</button></td>
														</tr>
											...
											<!-- <a href="#" class="modalClose" rel="modal:close">닫기</a> -->
							...
							<div class="biddingBtn_wrap fr">
								...
								<form onsubmit="return bid()" method="post"
									action="${pageContext.request.contextPath }/gall/bid"
									style="display: inline;">
									<input type="hidden" name="memberId"
										value="${loggedMember.memberId }" /> <input type="hidden"
										name="productId" value="${product.productId }" /> <input
										type="hidden" name="bidPrice" value="${product.unit }" /> <input
										type="hidden" name="curPrice" value="${sum }" /> <input
										type="submit" id="bidding_btn" value="입찰하기">


								</form>
								<form onsubmit="return now();" method="post"
									action="${pageContext.request.contextPath }/gall/winningBid"
									style="display: inline;">

									<input type="hidden" name="memberId" id="id"
										value="${loggedMember.memberId }" /> <input type="hidden"
										name="seller" id="seller" value="${product.productOwner }" />
									<input type="hidden" name="name" id="name"
										value="${product.productName }" /> <input type="hidden"
										name="price" id="price" value="${product.buyNowPrice }" /> <input
										type="hidden" name="image" id="image"
										value="${image[0]['rethumbnail']}" /> <input type="hidden"
										name="productId" id="productNo" value="${product.productId }" />
				...
				<div class="productContent_wrap">
							...
							<ul class="comment_table">
								<c:forEach var="c" items="${comment }">
									...
									<div class="comment_wrap">
										...
										<span> <a href="#">${c.commentContent }</a>
										</span>
									</div>
									...
									<c:if test="${loggedMember.memberId == c.memberId }">
										<div class="comment_btn">
											<span>
												<button class="delete" type="button">댓글삭제</button> <input
												type="hidden" id="pcmtNo" value="${c.commentNo }" /> <input
												type="hidden" id="groupId" class="groupId"
												value="${c.groupId }" />
											</span>
										</div>
									</c:if>
					...
					<form method="post" action="${path }/gall/productCommentInsert.do"
						id="comment_form">
						<div style="margin-top: 55px;">
							<input type="hidden" id="productId" name="productId"
								value="${product.productId }" /> <input type="hidden"
								id="memberId" name="memberId" value="${loggedMember.memberId }" />

							<textarea name="commentContent" id="commentContent" rows="6"
								cols="50"></textarea>
...
<script>
	$(".messageBtn").click(function(){
		 var id = $(this).parents("tr").children(".pMemberId").html();
		 var forder = "write";
		 /* window.open("${path }/member/messagePage?id="+id+"&forder="+forder ,"네이버 새창" ,"width=760,height=500"); */
		 window.open("${path }/member/messagePage?id="+id+"&forder="+forder, "네이버새창", "width=800, height=700, toolbar=no, menubar=no, scrollbars=no, resizable=yes" );  
	...
	$(".comments").click(
			function() {
				...
				$(this).parent().parent().append(
						...
						"<textarea name='commentContent' id='nserContent' rows='6' cols='50'>"
								+ "</textarea>"
								+ "<div class='comment_submit'>"
								+ "<button type='button' onclick='nser()'>"
								+ "등록" + "</button>" + "</div>" + "</div>"
	...
	function bidInformation() {
		var html = "${pageContext.request.contextPath}/gall/ppap?productId="
				+ productId;
